...
export default function SignUpScreen({navigation}) {
    return (
      <View className="flex-1 items-center justify-start bg-white pt-64 px-6">
       ...
       {/* Google Login Button */}
        ...
        <TouchableOpacity className="flex-row items-center justify-center w-full bg-green-600 rounded-xl mt-6 py-3">
          <Text className = "text-white text-base font-medium font-bold">Login</Text>
        </TouchableOpacity>
  
        {/* Dont have account? */}
  
        <Text className="text-xl mt-2 text-green-600 pt-8">Dont have an account?</Text>
  
        {/* Sign Up button */}
        <TouchableOpacity className = "flex-row items-center justify-center w-full bg-white-600 rounded-xl mt-6 py-3"
        onPress={() => navigation.navigate('SignUp')}>
        <Text className="text-green-600 text-base font-medium underline">Sign Up</Text>
